...
def migrate_data(apps, schema_editor):
    ...
    with connection.cursor() as cursor:
        ...
        # Миграция лейблов
        cursor.execute("""
            INSERT IGNORE INTO labels (name, country)
            SELECT DISTINCT name, country FROM public.labels;
        """)

        # Миграция исполнителей
        cursor.execute("""
            INSERT IGNORE INTO artists (name, country)
            SELECT DISTINCT name, country FROM public.artists;
        """)

        # Миграция альбомов
        cursor.execute("""
            INSERT IGNORE INTO albums (id, title, release_date, price, label_id, genre_id)
        ...
        # Миграция треков
        cursor.execute("""
            INSERT IGNORE INTO tracks (album_id, title, duration)
            SELECT album_id, title, duration FROM public.tracks;
        """)

        # Миграция клиентов
        cursor.execute("""
            INSERT IGNORE INTO customers (id, first_name, last_name, email, phone, address, city, country, registered_date)
        ...
        # Миграция заказов
        cursor.execute("""
            INSERT IGNORE INTO orders (id, customer_id, order_date, total_amount)
        ...
        cursor.execute("""
            INSERT IGNORE INTO order_items (order_id, album_id, quantity, price_at_purchase)
        ...
        cursor.execute("""
            INSERT IGNORE INTO employees (id, first_name, last_name, email, phone, position)
        ...
        # Миграция продаж
        cursor.execute("""
            INSERT IGNORE INTO sales (order_id, employee_id, sale_date)
        ...
        # Миграция платежей
        cursor.execute("""
            INSERT IGNORE INTO payments (order_id, payment_date, amount, payment_method)
        ...
        cursor.execute("""
            INSERT IGNORE INTO suppliers (id, name, contact_name, phone, email, country)
        ...
        # Миграция поставок
        cursor.execute("""
            INSERT IGNORE INTO shipments (supplier_id, album_id, quantity, shipment_date)
        ...
        # Миграция склада
        cursor.execute("""
            INSERT IGNORE INTO inventory (album_id, stock_quantity, last_updated)
